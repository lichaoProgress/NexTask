...
class DBManager(metaclass=SingletonMetaCls):
        ...
        *,
        ...
        Args:
            table_objs: orm映射类实例列表
                e.g. [UserTable(username="lc", age=18), ...] or [{"username": "lc", "age": 18}, ...]
            orm_table: orm表映射类
            flush: 刷新对象状态，默认不刷新
            session: 数据库会话对象，如果为 None，则通过装饰器在方法内部开启新的事务
    ...
    async def add(
        ...
        插入一条数据
        Args:
            table_obj: orm映射类实例对象, eg. UserTable(username="lc", age=18) or {"username": "lc", "age": 18}
            orm_table: orm表映射类
            session: 数据库会话对象，如果为 None，则通过装饰器在方法内部开启新的事务
    ...
    async def query_one(
        ...
        *,
        ...
        Examples:
            ...
            ret => UserTable(id=1, username="lc", age=18)
        ...
        if cols:
            if flat and len(cols) == 1:
                ...
                # eg: select username from user where id=1 从 {"username": "lc"} => "lc"
                return cursor_result.scalar_one_or_none()

            # eg: select username, age from user where id=1 => {"username": "lc", "age": 18}
            ret = cursor_result.mappings().one_or_none() or {}
            return SerializerUtil.model_to_data(ret)
        else:
            # 未指定列名查询默认全部字段，返回的是表实例对象 BaseOrmTable()
            # eg: select id, username, age from user where id=1 => UserTable(id=1, username="lc", age=18)
            if join_tables:
                # 连表还是返回 dict
                ret = cursor_result.mappings().one_or_none() or {}
                return SerializerUtil.model_to_data(ret)
    ...
    async def query_all(
        ...
        *,
        ...
        if cols:
            ...
            # eg: select username, age from user => [{"username": "lc", "age": 18}, [{"username": "dbk", "age": 18}]]
            ...
            # [User(id=1, username="lc", age=18), User(id=2, username="dbk", age=18)
            return cursor_result.scalars().all() or []
    ...
    async def update(
        ...
        Args:
            ...
            orm_table: ORM表映射类
            conds: 更新条件列表，每个条件为一个表达式，例如 [UserTable.username == "lc", ...]
        ...
        **kwargs,
        ...
        Args:
            table_obj: 映射类实例对象 or dict，
                e.g. UserTable(username="lc", age=18) or {"username": "lc", "v": 18, ...}
            orm_table: ORM表映射类
            session: 数据库会话对象，如果为 None，则在方法内部开启新的事务
    ...
    async def run_sql(
        ...
        Args:
            sql: sql语句
            params: sql参数, eg. {":id_val": 10, ":name_val": "lc"}
